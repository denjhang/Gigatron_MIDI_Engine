# Gigatron_MIDI_Engine
The Gigatron MIDI sound engine, written in gtbasic, supports simple macro-defined instruments. It converts MIDI to a `sound_on` function command stream; while this method is generally inefficient, it allows you to easily control four audio channels to emit sound simultaneously and enables various sound effects.
...
#### Macro Event Generation Algorithm
```cpp
// Generate macro sequence events for each Note On event
if (duration_ticks > 0) {
    // Get macro sequences
    ...
    // Calculate time interval for each macro event
    long tick_increment = static_cast<long>(std::round(60.0 / current_effective_accuracy));
    
    // Generate macro sequence events during note duration
...
#### Release Macro Processing
```cpp
// Generate macro sequence events after note release
